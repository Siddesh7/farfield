...
// GET /api/products/featured - Get top 3 featured products across all categories (Public)
async function getFeaturedProductsHandler(request: Request) {
  await connectDB();

  try {
    const featuredProducts = await getFeaturedProductsWithMetadata();
...
export const GET = withErrorHandling(getFeaturedProductsHandler);
